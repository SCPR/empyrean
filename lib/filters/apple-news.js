...
let htmlToBodyComponent = (html) => {
  ...
  if(component.text.length) {
    return component;  // Apple News doesn't like empty bodies.
  }
}
...
let embedPlaceholderToComponent = (element) => {
  ...
  if (url && role && (["twitter", "instagram", "youtube", "facebook"].indexOf(role) !== -1)) {
    if (role == "twitter") {
      role = "tweet";
...
function unwrapEmbedPlaceholders (html) {
  ...
  return new Promise((resolve, reject) => {
    filters.cleanup(article.body)
      ...
      .then((html) => {
        ...
        html = unwrapEmbedPlaceholders(html);
