...
module.exports = class {
    ...
    return this.getNewMessages()
      ...
      .then(messages => {
        ...
        return this.routeMessages(messages)
          .then(() => {
            performLater(() => { return this.perform(); }, this.config.sqs.message_queue_interval);
          })
      })
      .catch(err => {
  ...
  routeMessages (messages) {
    ...
    messages.forEach(message => {
      promise = promise.then(() => this.routeMessage(message));
    });
    return promise;
  }
  ...
  routeMessage (message) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if(validationResult.errors.length) return reject(validationResult);
      ...
      return this.db.upsert(messageId, this._deltaFunc)
        ...
        .then(doc => {
          ...
          return adapter(message)
            .then(resp => {
              ...
              // Choose the proper logger depending on the returned status code.
              ...
              Object.keys(resp).forEach(key => syndication[key] = resp[key]);
              syndication.checksum  = message.MD5OfBody;
              syndication.timestamp = new Date();
        ...
        .catch(err => {
          ...
          newErr.message = `${messageId}:\n${newErr.message}`; // prepend the message ID to the error message and then pass it on
          reject(newErr);
        });
    })
  }
